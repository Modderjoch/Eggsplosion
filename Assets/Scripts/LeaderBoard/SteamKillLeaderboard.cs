...
public class SteamKillLeaderboard : MonoBehaviour
{
    private const string s_leaderboardName = "All Time Kills";
    ...
    private static SteamLeaderboard_t s_killboard;
    ...
    {     
        if (finishedDownloading)
        {
            if (isRoundOver && RoundOverManager != null)
            {
                ...
                UpdateScore(RoundOverManager.playerConfigs[0].killAmount + downloadedScore);
            }
            else if (!isRoundOver && ResultsManager != null)
            {
                Debug.Log(ResultsManager.playerConfigs[0].playerInput.devices[0].deviceId);
                Debug.Log(downloadedScore + " downloaded score!");
                UpdateScore(ResultsManager.playerConfigs[0].killAmount + downloadedScore);
    ...
    public void DownloadSteamLeaderboardEntries()
    {
        ...
        SteamAPICall_t hSteamAPICall = SteamUserStats.DownloadLeaderboardEntries(s_killboard, ELeaderboardDataRequest.k_ELeaderboardDataRequestGlobalAroundUser, -1, 1);
    ...
    public void UpdateScore(int score)
    {
        ...
        else
        {
            ...
            SteamAPICall_t hSteamAPICall = SteamUserStats.UploadLeaderboardScore(s_killboard, s_leaderboardMethod, score, null, 0);
    ...
    static private void OnLeaderboardFindResult(LeaderboardFindResult_t pCallback, bool failure)
    {
        ...
        s_killboard = pCallback.m_hSteamLeaderboard;
